...
public class DamerauLevenshteinDistance {
    ...
    private static int DamerauLevenshtein(int[][] ds, int i, int j, String a, String b) {
        if (Math.min(i, j) == 0) return Math.max(i, j);
                    ...
                    ds[i - 1][j] + 1,
                    ds[i][j - 1] + 1,
                    ds[i - 1][j - 1] + (a.charAt(i) == b.charAt(j) ? 0 : 1),
                    (i > 1 && j > 1 && a.charAt(i) == b.charAt(j - 1) && a.charAt(i - 1) == b.charAt(j)) ? ds[i - 2][j - 2] + 1 : Integer.MAX_VALUE
            );
    }

    private static int WagnerFischer(String a, String b) {
        int[][] ds = new int[a.length()][b.length()];
        for (int i = 0; i < a.length(); i++) {
            for (int j = 0; j < b.length(); j++) {
                ds[i][j] = DamerauLevenshtein(ds, i ,j, a, b);
            }
        }
        return ds[a.length() - 1][b.length() - 1];
    }
